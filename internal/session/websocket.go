...
func (m *Manager) HandleSession(c *gin.Context) {
	...
	log.Printf("websocket client connected, session id: %s", sessionID)
	...
	for {
		...
		log.Printf("session %s: received command: %s", sessionID, req.Command)
	...
	log.Printf("session %s: closing", sessionID)
}
